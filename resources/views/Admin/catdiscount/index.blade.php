                            ...
                            <div class="box-body">
                                <div class="table-responsive">
                                    <table id="example5" class="table table-bordered table-striped" style="width:100%">
                                        ...
                                        <tbody>
                                        @foreach($catdiscounts as $catdiscount)
                                            <tr>
                                                <th>{{$catdiscount->id}}</th>
                                                <th>{{$catdiscount->discount}}</th>
                                                <th>
                                                    {{$catdiscount->show_category->title}}
                                              </th>
                                                <th>{{$catdiscount->created_at}}</th>
                                                <th><a class="btn btn-outline-info"
                                                       href="{{route('catdiscounts.edit',$catdiscount)}}">ویرایش</a></th>
                                                <th>
                                                    <form method="post" action="{{route('catdiscounts.destroy',$catdiscount)}}">
                                                        @csrf
                                                        @method('DELETE')
